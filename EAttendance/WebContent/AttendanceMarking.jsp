...
<s:if test="listStudent.size() == 0">
<TR>
	<TD class="label" align="center"><font="Red">No Recors Found</font></TD></TR>
</s:if>
<s:if test="listStudent.size() > 0">

<s:iterator value="listStudent" status="studentStatus">

<TR>
	<TD class="label" align="center"><s:property value="id" /></TD>
	<TD class="label" align="center"> <s:property value="roll_no" /></TD>
	<TD class="label" align="center"><s:property value=" first_name "/></TD>
	<TD>
	 <s:checkbox name ="attendanceMarking"  id="%{id}"  fieldValue="%{id}" value="%{id}"  />
	 <s:hidden name="recCount" value="%{id}" />
	</TD>
	<%-- <s:hidden name="listStudentAttendance[%{#attendance.index}].id" value=%{id}/> --%>
...
<TR>
<TD align="center">	<s:submit action="markAttendance"  value="Mark Attendance" /></TD>
</TR>
</s:if>
</TABLE>
</s:form>
